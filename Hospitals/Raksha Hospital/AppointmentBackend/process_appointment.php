...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT * FROM Rakshahospital WHERE doctor_name = ? AND appointment_date = ? AND appointment_time = ?");
    ...
    $stmt = $conn->prepare("INSERT INTO Rakshahospital (patient_name, patient_age, patient_phone, patient_email, test_type, doctor_name, appointment_date, appointment_time, payment_method, amount) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Appointment booked successfully!'); window.location.href='http://localhost/final%20year%20project-2025/Hospitals/Raksha%20Hospital/';</script>";
    } else {
