...
$stmt = $conn->prepare("SELECT * FROM cardiac_tests WHERE test_center = ? AND test_date = ? AND test_time = ?");
...
$stmt = $conn->prepare("INSERT INTO cardiac_tests (patient_name, patient_age, patient_phone, patient_email, test_type, test_center, test_date, test_time, payment_method, amount) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
if ($stmt->execute()) {
    echo "<script>alert('Appointment booked successfully!'); window.location.href='http://localhost/final%20year%20project-2025/DIagnostic%20center%20Files/MAINPAGE/CARDIACTEST/cardiacTest.html';</script>";
} else {
